...
function invokeApi(config, apiOptions) {
    const options = {
        method: apiOptions.method,
        json: false,
        headers: apiOptions.headers,
        url: config.environment.apiEndpoint,
...
/* Expected options:
       ...
       options.headers
*/
function signHeaders(config, options) {
    const headers = {};

    Object.assign(headers, options.headers);

    // headers['x-amz-pay-region'] = config.region;
    headers['x-amz-pay-host'] =  urlLib.parse(config.environment.apiEndpoint).hostname;
    ...
    if (payload === null || payload === undefined || config.environment.apiEndpoint.includes('/account-management/v1/accounts')) {
            payload = ''; // do not sign payload for payment critical data APIs
    }

    let canonicalRequest = options.method + '\n' + urlLib.parse(config.environment.apiEndpoint).pathname + '\n\n';
