...
exports.notifications = this.notifications;
...
notifications.prototype.buildPayload = function(orderReferenceId, trackingNumber, carrierCode) {
  ...
  // TODO do better check of OR id
  if (orderReferenceId[0] !== "P" &&
    orderReferenceId[0] !== "S") {
...
notifications.prototype.sendDeliveryNotification = async function(payload, configArgs) {
  ...
  let response;
  try {
    const preparedOptions = Helper.prepareOptions(configArgs, options);
    preparedOptions.headers = Helper.signHeaders(configArgs, preparedOptions);
    response = await Helper.invokeApi(configArgs, preparedOptions);
  } catch (error) {
    console.log(error);
  }
