...
function invokeApi(configArgs, apiOptions) {
    const options = {
        ...
        url: configArgs.environment.apiEndpoint,
...
function signHeaders(configArgs, options) {
    ...
    headers['x-amz-pay-host'] =  urlLib.parse(options.url).hostname;
    ...
    if (payload === null || payload === undefined || options.url.includes('/account-management/v1/accounts')) {
            payload = ''; // do not sign payload for payment critical data APIs
    }

    let canonicalRequest = options.method + '\n/' + urlLib.parse(options.url).pathname + '\n\n';
