...
function parseSNSResponse(response, callback) {
  ...
  for (var i = 0; i < required.length; i ++) {
    if (!response.hasOwnProperty(required[i])) {
      return callback(err.missingParameter('Missing parameter on SNS response: ' + required[i]));
    }
  }

  if (response.SignatureVersion != 1) {
    return callback(err.invalidSignatureVersion('Unknown SNS Signature version: ' + response.SignatureVersion));
  }
  ...
  request(response.SigningCertURL, function(err, res, cert) {
    ...
    if (!isValid) {
      return callback(err.signatureMismatch('Signature mismatch, unverified response'));
    }
...
function parseIPNMessage(message, callback) {
  ...
  if (Object.prototype.toString.call(message) != '[object Object]' || !message.NotificationData) {
